...
// Clickwrap acceptance gate. Blocks the entire app on first launch until the
...
export function TermsGate({ children }: { children: React.ReactNode }) {
  // null = still reading localStorage (avoid flicker)
  ...
  useEffect(() => {
    try {
      ...
      // localStorage blocked — fail closed: show the gate.
      setAccepted(false);
    }
  }, []);
      ...
      >
        <div className="w-full max-w-lg rounded-xl border border-border bg-card p-6 sm:p-8 shadow-2xl">
          ...
          <div className="mt-5 space-y-4 text-sm text-muted-foreground">
            ...
            <div className="rounded-lg bg-[hsl(37,10%,80%)] dark:bg-muted p-4">
          ...
          <label className="mt-6 flex items-start gap-3 cursor-pointer select-none">
            ...
            <span className="text-sm text-card-foreground">
              ...
              <button type="button" onClick={() => openLegal('/terms')} className="underline text-primary hover:opacity-80">Terms of Service</button>
              {' '}and{' '}
              <button type="button" onClick={() => openLegal('/privacy')} className="underline text-primary hover:opacity-80">Privacy Policy</button>,
